...
Splunk.Module.RuleResultsTable = $.klass(Splunk.Module.AbstractPagedModule, {
    ...
    onLoadStatusChange: function($super, statusInt) {
        ...
        if (statusInt == Splunk.util.moduleLoadStates.HAS_CONTEXT) {
            ...
            if (context === null) {
                this.logger.warn('getContext could not retrieve a context object when called from RuleResultsTable\'s onLoadStatusModule.');
                return;
            }
    ...
    /**
     * RuleResultsTable can have child modules.
    ...
    onResultsRendered: function() {
        //this.logger.debug('RuleResultsTable.onResultsRendered - START');
        var context = this.getContext();
	...
	//Define uncheck functionality
	$("#clearButton", this.container).click(function() {
		...
		$("> tr > td.alarm_selector > input.alarm_checkbox", $(this).parent().parent().parent()).prop('checked', !checked);
	});

        var severity_name = "ALERT_SEVERITY";
        var severity_index = -1;
        var headers = $("tr .sortLabel");
        for (var i = 0; i < headers.length; i++)
        {
            if (headers[i].innerHTML == severity_name) severity_index = i + 2;
        }
            
        if (severity_index != -1) {
            $("tr:has(td)", this.container).each(function() {
                var element = $(this).find("td:nth-child(" + severity_index + ")");
