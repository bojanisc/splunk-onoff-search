...
# logging setup
import logging
logger = logging.getLogger('splunk.module.RuleResultsTable')
...
#define alarm BLOCK_ID field
BLOCK_ID_FIELD = 'REFERENTNI_BROJ'
...
class RuleResultsTable(module.ModuleHandler):
    '''
    Provides module content for the RuleResultsTable.  The arguments supported 
    ...
    def generateResults(self, host_app, client_app, sid, count=1000, 
        ...
        if not sid:
            raise Exception('RuleResultsTable.generateResults - sid not passed!')
        ...
        try:
            ...
            logger.error('RuleResultsTable could not find the job %s. Exception: %s' % (sid, e))
        ...
        if len(ordered):
            ...
            logger.debug('RuleResultsTable - localMin=%s localMax=%s adjustedMin=%s adjustedMax=%s' % (localMin, localMax, adjustedMin, adjustedMax))
        ...
        #Add header for checkbox selectors
        output.append('<th class="alarm_selector"><button id="clearButton" type="button"></button></th>')
        
        for field in fieldNames:
            output.append('<th><a><span class="sortLabel">%s</span> <span class="splSort%s"></span></a></th>' % (cgi.escape(field), field != sortField and "None" or sortDir))
        ...
        for i, result in enumerate(dataset):
            ...
            #Add checkbox selectors
            block_id_field_value = result.get(BLOCK_ID_FIELD, None)
            if block_id_field_value is not None:
                output.append('<td class="alarm_selector"><input class="alarm_checkbox" type="checkbox" name="selector" blockid="%s"></td>' % block_id_field_value)
